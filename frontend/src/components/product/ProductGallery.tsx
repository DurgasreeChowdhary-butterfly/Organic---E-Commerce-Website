...
export default function ProductGallery({ tint, name, images = [] }: ProductGalleryProps) {
  ...
  const activeFailed = failedIndices.has(active);
    ...
    <div>
      ...
      >
        {activeUrl && !activeFailed ? (
          <img src={activeUrl} alt={name} onError={() => markFailed(active)} className="w-full h-full object-cover" />
        ) : (
      ...
      {sorted.length > 1 && (
        <div className="grid grid-cols-4 gap-3">
          {sorted.map((img, i) => {
            ...
            const thumbFailed = failedIndices.has(i);
            return (
              <button
                key={img.id}
                onClick={() => setActive(i)}
              ...
              >
                {thumbUrl && !thumbFailed ? (
                  <img src={thumbUrl} alt="" onError={() => markFailed(i)} className="w-full h-full object-cover" />
                ) : (
