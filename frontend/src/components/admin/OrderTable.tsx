...
export default function OrderTable() {
  const [statuses, setStatuses] = useState<Record<string, OrderStatus>>(
    Object.fromEntries(ADMIN_ORDERS.map((o) => [o.id, o.status]))
  );
            ...
            {ADMIN_ORDERS.map((o) => (
              <tr key={o.id} className="border-b border-beige/60 last:border-0 hover:bg-pista-50/40">
                ...
                <td className="px-5 py-3">
                  <select
                    value={statuses[o.id]}
                    onChange={(e) => setStatuses((prev) => ({ ...prev, [o.id]: e.target.value as OrderStatus }))}
                    style={{ color: STATUS_COLOR[statuses[o.id]] }}
                ...
                <td className="px-5 py-3 text-right">
                  <button className="p-2 rounded-lg hover:bg-pista-50 text-forest-700 inline-flex" aria-label="Download invoice">
