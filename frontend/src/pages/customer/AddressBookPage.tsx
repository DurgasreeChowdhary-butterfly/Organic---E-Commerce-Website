    ...
    return (
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-8">
        <div className="grid sm:grid-cols-2 gap-4">
          {[0, 1].map((i) => (
            <div key={i} className="rounded-3xl bg-white shadow-soft p-5 space-y-3">
            ...
            return (
              <div key={addr.id} className="rounded-3xl bg-white shadow-soft p-5 animate-fade-up">
                ...
                <div className="flex items-center gap-4 text-xs font-semibold">
                  <button onClick={() => openEdit(addr)} disabled={busy} className="flex items-center gap-1 text-forest-700 hover:text-pista-700 disabled:opacity-50"><Pencil className="w-3.5 h-3.5" /> Edit</button>
                  <button onClick={() => setDeleteTarget(addr)} disabled={busy} className="flex items-center gap-1 text-brown-500 hover:text-red-600 disabled:opacity-50"><Trash2 className="w-3.5 h-3.5" /> Delete</button>
                  {!addr.is_default && (
